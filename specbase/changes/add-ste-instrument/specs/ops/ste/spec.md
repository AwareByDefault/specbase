...
<!--
  Ops adoption pair, planted opt-in at init. It declares the writing standard the
  repo adopts and the tool it adopts to enforce it. It names the linter by its
  role ("the adopted STE tool") and references the command contract by locator
  (behavior.cli.ste-lint); it does not restate that contract.
-->

## ADDED Requirements

### Requirement: Specbase prose follows Simplified Technical English
**ID:** ste-is-the-writing-standard
Specbase SHALL write its user-facing and internal prose — READMEs, skill docs,
and internal design docs — in Simplified Technical English: short active
...
- **WHEN** a Specbase README, skill doc, or internal doc is authored or revised
...
Specbase today gates `README.md` plus `docs/**/*.md` at a `--max` threshold of
`8` total-per-100-words — measured against the real backlog, whose top document
sits around 5.5, so the gate fails only when prose pushes past 8, not on the
current docs. The glob and the number are scope decisions recorded here, revisited

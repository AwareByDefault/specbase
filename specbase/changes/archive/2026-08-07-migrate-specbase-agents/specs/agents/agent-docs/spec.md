...
<!--
  Agents truth. The operational artifact is the root `AGENTS.md` — the file
  agents actually load when they work in this repository. This spec DESCRIBES
  that file and its enforcement binds conformance/drift checks against it; the
  file stays the runtime source of truth and is never generated from this spec.

  Migrated from the legacy `docs-agent-instructions` capability (6 kept
  requirements, mapping.md Part A). The craft rules the docs must TEACH
  (behavior-first boundary, progressive rigor) live in
  `code-quality/spec-authoring`; this pair only requires that the docs teach
  them.
-->

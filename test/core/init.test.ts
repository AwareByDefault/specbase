...
describe('InitCommand', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `specbase-init-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid reading real config
    configTempDir = path.join(os.tmpdir(), `specbase-config-init-${Date.now()}`);
  ...
  describe('execute with --tools flag', () => {
    it('should create Specbase directory structure', async () => {
      const initCommand = new InitCommand({ tools: 'claude', force: true });

      await initCommand.execute(testDir);

      const specbasePath = path.join(testDir, 'specbase');
      expect(await directoryExists(specbasePath)).toBe(true);
    ...
    it('should create skills in Cursor skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.cursor', 'skills', 'specbase-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills in Windsurf skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.windsurf', 'skills', 'specbase-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should support Kimi CLI as an adapterless skills-only tool', async () => {
      ...
      const skillFile = path.join(testDir, '.kimi', 'skills', 'specbase-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    ...
    it('should create both skills and commands for Trae with adapter', async () => {
      ...
      const skillFile = path.join(testDir, '.trae', 'skills', 'specbase-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    ...
    it('should create skills for multiple tools at once', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'specbase-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'specbase-explore', 'SKILL.md');
    ...
    it('should select all tools with --tools all option', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'specbase-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'specbase-explore', 'SKILL.md');
      const windsurfSkill = path.join(testDir, '.windsurf', 'skills', 'specbase-explore', 'SKILL.md');
    ...
    it('should skip tool configuration with --tools none option', async () => {
      ...
      // Should create Specbase structure but no skills
      const specbasePath = path.join(testDir, 'specbase');
      expect(await directoryExists(specbasePath)).toBe(true);
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const specbasePath = path.join(newDir, 'specbase');
      expect(await directoryExists(specbasePath)).toBe(true);
    });
  ...
  describe('skill content validation', () => {
    it('should generate valid SKILL.md with YAML frontmatter', async () => {
      ...
      expect(content).toContain('name: specbase-explore');
      expect(content).toContain('description:');
    ...
    it('should include propose skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'specbase-propose', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: specbase-propose');
    });
    ...
    it('should include apply-change skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'specbase-apply-change', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: specbase-apply-change');
    });
      ...
      vi.spyOn(fs, 'writeFile').mockImplementation(
        async (filePath: any, ...args: any[]) => {
          if (
            ...
            filePath.includes('.specbase-test-')
          ) {
...
describe('InitCommand - profile and detection features', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `specbase-init-profile-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid polluting real config
    configTempDir = path.join(os.tmpdir(), `specbase-config-test-${Date.now()}`);
    await fs.mkdir(configTempDir, { recursive: true });
    process.env.XDG_CONFIG_HOME = configTempDir;
    vi.spyOn(console, 'log').mockImplementation(() => {});
    confirmMock.mockReset();
  ...
  it('should use --profile flag to override global config', async () => {
    ...
    const proposeSkill = path.join(testDir, '.claude', 'skills', 'specbase-propose', 'SKILL.md');
    expect(await fileExists(proposeSkill)).toBe(true);

    // Non-core skills (from the custom profile) should NOT be created
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'specbase-new-change', 'SKILL.md');
  ...
  it('should preselect configured tools but not directory-detected tools in extend mode', async () => {
    // Simulate existing Specbase project (extend mode).
    await fs.mkdir(path.join(testDir, 'specbase'), { recursive: true });

    // Configured with Specbase
    ...
    // Directory detected only (not configured with Specbase)
  ...
  it('should preselect detected tools for first-time interactive setup', async () => {
    // First-time init: no specbase/ directory and no configured Specbase skills.
    await fs.mkdir(path.join(testDir, '.github'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.github', 'copilot-instructions.md'), '');
  ...
  it('should respect custom profile from global config', async () => {
    ...
    const exploreSkill = path.join(testDir, '.claude', 'skills', 'specbase-explore', 'SKILL.md');
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'specbase-new-change', 'SKILL.md');

...
describe('project-config', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-test-config-'));
...
`
    ...
    describe('.yml/.yaml precedence', () => {
      ...
      it('should return null when specbase directory does not exist', () => {
        const config = readProjectConfig(tempDir);
  ...
  describe('suggestSchemas', () => {
    ...
    it('should include fix instruction', () => {
      ...
      expect(message).toContain(
        "Fix: Edit specbase/config.yaml and change 'schema: wrong-schema' to a valid schema name"
      );
    });

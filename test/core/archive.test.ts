...
describe('ArchiveCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `specbase-archive-test-${Date.now()}`);
    ...
    // Create Specbase structure
    const specbaseDir = path.join(tempDir, 'specbase');
    await fs.mkdir(path.join(specbaseDir, 'changes'), { recursive: true });
    await fs.mkdir(path.join(specbaseDir, 'specs'), { recursive: true });
    await fs.mkdir(path.join(specbaseDir, 'changes', 'archive'), { recursive: true });
  ...
  describe('execute', () => {
    it('should archive a change successfully', async () => {
      ...
      const changeDir = path.join(tempDir, 'specbase', 'changes', changeName);
      ...
      const archiveDir = path.join(tempDir, 'specbase', 'changes', 'archive');
    ...
    it('detects incomplete tasks in nested glob tasks.md files (#1202 data-safety gate)', async () => {
      ...
      const schemaDir = path.join(tempDir, 'specbase', 'schemas', 'glob-tasks');
...
Then expected result happens`;
      ...
      const mainSpecPath = path.join(tempDir, 'specbase', 'specs', 'test-capability', 'spec.md');
...
### Requirement: Thumbnail Field`;
      ...
      const mainSpecPath = path.join(tempDir, 'specbase', 'specs', 'gift-card', 'spec.md');
      const updatedContent = await fs.readFile(mainSpecPath, 'utf-8');
...
Modified content.`;
      ...
      const mainSpecPath = path.join(tempDir, 'specbase', 'specs', 'new-capability', 'spec.md');
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, 'specbase', 'specs', 'another-capability', 'spec.md');
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
    ...
    it('should throw error if archive already exists', async () => {
      ...
      const archivePath = path.join(tempDir, 'specbase', 'changes', 'archive', `${date}-${changeName}`);
...
Then expected result happens`;
    ...
    it('should support header trim-only normalization for matching', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'specbase', 'specs', 'alpha');
...
Updated details.`;
    ...
    it('should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'specbase', 'specs', 'beta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it('should abort with error when MODIFIED/REMOVED reference non-existent requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'specbase', 'specs', 'gamma');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Another Missing`;
    ...
    it('should abort stale MODIFIED blocks that would drop current scenarios (issue #1246)', async () => {
      const mainSpecDir = path.join(tempDir, 'specbase', 'specs', 'stale-modified');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** it succeeds`;
      ...
      const changeADir = path.join(tempDir, 'specbase', 'changes', changeA);
...
- **THEN** it succeeds`);
      ...
      const changeBDir = path.join(tempDir, 'specbase', 'changes', changeB);
...
- **THEN** it succeeds`);
    ...
    it('should abort with a structural error when target spec hides requirements outside ## Requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'specbase', 'specs', 'delta-target');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** qux changes`;
    ...
    it('should require MODIFIED to reference the NEW header when a rename exists (error format)', async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, 'specbase', 'specs', 'delta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
new body`;
    ...
    it('should process multiple specs atomically (any failure aborts all)', async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, 'specbase', 'specs', 'epsilon', 'spec.md');
...
e1`);
      ...
      const zetaMain = path.join(tempDir, 'specbase', 'specs', 'zeta', 'spec.md');
...
### Requirement: Missing`);
    ...
    it('should display aggregated totals across multiple specs', async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, 'specbase', 'specs', 'omega', 'spec.md');
      await fs.mkdir(path.dirname(omegaMain), { recursive: true });
      await fs.writeFile(omegaMain, `# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1`);

      const psiMain = path.join(tempDir, 'specbase', 'specs', 'psi', 'spec.md');
...
Modified content.`;
    ...
    it('sets exit code 1 when rebuilt spec fails validateSpecContent', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'specbase', 'specs', 'rebuilt-capability');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
- **THEN** z`;
  ...
  describe('error handling', () => {
    it('should report no active changes when specbase directory does not exist', async () => {
      // Remove specbase directory
      await fs.rm(path.join(tempDir, 'specbase'), { recursive: true });
  ...
  describe('interactive mode', () => {
    it('should use select prompt for change selection', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'specbase', 'changes', change1), { recursive: true });
      await fs.mkdir(path.join(tempDir, 'specbase', 'changes', change2), { recursive: true });

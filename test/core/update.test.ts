...
import { LEGACY_OPENSPEC_MARKERS } from '../../src/core/config.js';
...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    // Create specbase directory
    const specbaseDir = path.join(testDir, 'specbase');
    await fs.mkdir(specbaseDir, { recursive: true });
  ...
  describe('basic validation', () => {
    it('should throw error if specbase directory does not exist', async () => {
      // Remove specbase directory
      await fs.rm(path.join(testDir, 'specbase'), {
        recursive: true,
        force: true,
      });
...
`;
      ...
      expect(updatedSkill).toContain('name: specbase-explore');
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'specbase-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'specbase-explore', 'SKILL.md'),
        'old content'
      );
  ...
  describe('command updates', () => {
    it('should update spcb commands for configured Claude tool', async () => {
      ...
      // Check spcb command files were created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'spcb');
    ...
    it('should update core profile spcb commands when tool is configured', async () => {
  ...
  describe('multi-tool support', () => {
    it('should update multiple configured tools', async () => {
      ...
      await fs.mkdir(path.join(claudeSkillsDir, 'specbase-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'specbase-explore', 'SKILL.md'),
        'old'
      );

      // Set up Cursor
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'specbase-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(cursorSkillsDir, 'specbase-explore', 'SKILL.md'),
        'old'
      );
      ...
      expect(claudeSkill).toContain('name: specbase-explore');
      ...
      expect(cursorSkill).toContain('name: specbase-explore');
    ...
    it('should update Qwen tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(qwenSkillsDir, 'specbase-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(qwenSkillsDir, 'specbase-explore', 'SKILL.md'),
        'old'
      );

      await updateCommand.execute(testDir);

      // Check Qwen command format (TOML) - Qwen uses flat path structure: spcb-<id>.toml
    ...
    it('should update Windsurf tool with correct command format', async () => {
      ...
      await fs.mkdir(path.join(windsurfSkillsDir, 'specbase-explore'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(windsurfSkillsDir, 'specbase-explore', 'SKILL.md'),
        'old'
      );
  ...
  describe('skill content validation', () => {
    ...
    it('should include proper instructions in skill files', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'specbase-apply-change'), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(skillsDir, 'specbase-apply-change', 'SKILL.md'),
        'old'
      );
...
`
    ...
    it('should update all tools when force is used with mixed versions', async () => {
      ...
      const { version } = await import('../../package.json');
      const claudeSkillDir = path.join(testDir, '.claude', 'skills', 'specbase-explore');
...
`
      ...
      const cursorSkillDir = path.join(testDir, '.cursor', 'skills', 'specbase-explore');
...
`
  ...
  describe('version tracking', () => {
    ...
    it('should only update tools that need updating', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'specbase-explore', 'SKILL.md');
  ...
  describe('legacy cleanup', () => {
    it('should detect and auto-cleanup legacy files with --force flag', async () => {
      ...
      // Create legacy CLAUDE.md with Specbase markers
      const legacyContent = `${LEGACY_OPENSPEC_MARKERS.start}
# Specbase Instructions

These instructions are for AI assistants.
${LEGACY_OPENSPEC_MARKERS.end}
`;
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Upgrading to the new Specbase')
      );
      ...
      expect(content).not.toContain(LEGACY_OPENSPEC_MARKERS.start);
      expect(content).not.toContain(LEGACY_OPENSPEC_MARKERS.end);
...
`;
    ...
    it('should cleanup legacy specbase/AGENTS.md with --force', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'specbase-explore'), {
        recursive: true,
      ...
      // Create legacy specbase/AGENTS.md
      await fs.writeFile(
        path.join(testDir, 'specbase', 'AGENTS.md'),
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Removed specbase/AGENTS.md')
      );
    ...
    it('should not show legacy cleanup messages when no legacy files exist', async () => {
      ...
      const hasLegacyMessage = calls.some(call =>
        call.includes('Upgrading to the new Specbase')
      );
    ...
    it('should remove Specbase marker block from mixed content files', async () => {
      // Set up a configured tool
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'specbase-explore'), {
        recursive: true,
      ...
      // Create CLAUDE.md with mixed content (user content + Specbase markers)
      const mixedContent = `# My Project

Some user-defined instructions here.

${LEGACY_OPENSPEC_MARKERS.start}
# Specbase Instructions
...
`;
      ...
      expect(updatedContent).not.toContain(LEGACY_OPENSPEC_MARKERS.start);
      expect(updatedContent).not.toContain(LEGACY_OPENSPEC_MARKERS.end);
  ...
  describe('legacy tool upgrade', () => {
    it('should upgrade legacy tools to new skills with --force', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('/spcb:new')
      );

      // Skills should be created
      const skillFile = path.join(testDir, '.claude', 'skills', 'specbase-explore', 'SKILL.md');
    ...
    it('should upgrade multiple legacy tools with --force', async () => {
      ...
      const claudeSkillFile = path.join(testDir, '.claude', 'skills', 'specbase-explore', 'SKILL.md');
      const cursorSkillFile = path.join(testDir, '.cursor', 'skills', 'specbase-explore', 'SKILL.md');
    ...
    it('should not upgrade legacy tools already configured', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'specbase-explore'), { recursive: true });
      await fs.writeFile(
        path.join(skillsDir, 'specbase-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should upgrade only unconfigured legacy tools when mixed', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'specbase-explore'), { recursive: true });
      await fs.writeFile(
        path.join(claudeSkillsDir, 'specbase-explore', 'SKILL.md'),
        'existing skill'
      );
    ...
    it('should create only effective profile skills when upgrading legacy tools', async () => {
      ...
      const nonCoreSkill = path.join(skillsDir, 'specbase-new-change', 'SKILL.md');
    ...
    it('should create commands when upgrading legacy tools', async () => {
      ...
      // New spcb commands should be created
      const commandsDir = path.join(testDir, '.claude', 'commands', 'spcb');
      const exploreCmd = path.join(commandsDir, 'explore.md');
      const exists = await FileSystemUtils.fileExists(exploreCmd);
      expect(exists).toBe(true);
    ...
    it('should not inject non-profile workflows when upgrading legacy tools', async () => {
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'specbase-propose', 'SKILL.md')
      )).toBe(false);
  ...
  describe('profile-aware updates', () => {
    it('should generate only profile workflows when custom profile is set', async () => {
      ...
      const skillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(skillsDir, 'specbase-explore'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'specbase-explore', 'SKILL.md'), 'old');
      ...
      expect(await FileSystemUtils.fileExists(
        path.join(skillsDir, 'specbase-new-change', 'SKILL.md')
      )).toBe(true);
    ...
    it('should suggest core preset when custom profile preserves the old core workflow set', async () => {
      ...
      expect(calls.some(call =>
        call.includes('specbase config profile core') && call.includes('specbase update')
      )).toBe(true);

      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'skills', 'specbase-sync-specs', 'SKILL.md')
      )).toBe(false);
      expect(await FileSystemUtils.fileExists(
        path.join(testDir, '.claude', 'commands', 'spcb', 'sync.md')
      )).toBe(false);
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      ...
      await fs.mkdir(path.join(skillsDir, 'specbase-new-change'), { recursive: true });
      await fs.writeFile(path.join(skillsDir, 'specbase-new-change', 'SKILL.md'), 'old');
      const extraCommandFile = path.join(testDir, '.claude', 'commands', 'spcb', 'new.md');
  ...
  describe('new tool detection', () => {
    it('should detect new tool directories not currently configured', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'specbase-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'specbase-explore', 'SKILL.md'), 'old');
      ...
      const hasNewToolMessage = calls.some(call =>
        call.includes("Detected new tool: Cursor. Run 'specbase init' to add it.")
      );
    ...
    it('should consolidate multiple new tools into one message', async () => {
      ...
      expect(consolidatedCalls[0]).toContain("Run 'specbase init' to add them.");
  ...
  describe('scanInstalledWorkflows', () => {
    it('should detect installed workflows across tools', async () => {
      ...
      const claudeSkillsDir = path.join(testDir, '.claude', 'skills');
      await fs.mkdir(path.join(claudeSkillsDir, 'specbase-explore'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'specbase-explore', 'SKILL.md'), 'content');
      await fs.mkdir(path.join(claudeSkillsDir, 'specbase-apply-change'), { recursive: true });
      await fs.writeFile(path.join(claudeSkillsDir, 'specbase-apply-change', 'SKILL.md'), 'content');
    ...
    it('should return union of workflows across multiple tools', async () => {
      ...
      // Cursor has apply
      const cursorSkillsDir = path.join(testDir, '.cursor', 'skills');
      await fs.mkdir(path.join(cursorSkillsDir, 'specbase-apply-change'), { recursive: true });
      await fs.writeFile(path.join(cursorSkillsDir, 'specbase-apply-change', 'SKILL.md'), 'content');

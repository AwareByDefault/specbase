...
import type { ResolvedSpecbaseRoot } from '../../src/core/root-selection.js';
import { createSpecbaseRoot, writeSpec } from '../helpers/specbase-fixtures.js';
...
describe('reference index assembly', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-references-'));
    globalDataDir = path.join(tempDir, 'data', 'specbase');
  ...
  function appRoot(): ResolvedSpecbaseRoot {
    const rootDir = mkdir('app-repo');
    createSpecbaseRoot(rootDir);
    return {
      path: rootDir,
      source: 'nearest',
      changesDir: path.join(rootDir, 'specbase', 'changes'),
      defaultSchema: 'spec-driven',
    } as ResolvedSpecbaseRoot;
  }
  ...
  it('indexes a resolved store with first-Purpose-line summaries and the fetch recipe', async () => {
    ...
    expect(entry.fetch).toBe('specbase show <spec-id> --type spec --store team-context');
  ...
  it('degrades an unregistered reference to reference_unresolved with a pasteable fix', async () => {
    ...
    expect(entries[0].status[0]).toEqual(
      expect.objectContaining({
        ...
        fix: expect.stringContaining('specbase store register <path> --id missing-context'),
      })
    );
  });

  it('renders a verbatim clone fix when the declaration carries a remote (3.3)', async () => {
    const checkout = path.join(os.homedir(), 'specbase', 'missing-context');
    ...
    expect(entries[0].status[0].fix).toBe(
      `git clone -- https://192.0.2.1/team.git ${q}${checkout}${q} && specbase store register ${q}${checkout}${q} --id missing-context`
    );
    ...
    for (const entry of entries) {
      expect(entry.status[0]).toEqual(
        expect.objectContaining({
          ...
          fix: expect.stringContaining('specbase store doctor'),
        })
      );
    }
  });
  ...
  it('keeps registry-independent checks first under a corrupt registry', async () => {
    ...
    const entries = await assembleReferenceIndex({
      ...
      resolvedRoot: {
        ...
        source: 'store',
        storeId: 'self-store',
        changesDir: path.join(root, 'specbase', 'changes'),
        defaultSchema: 'spec-driven',
      } as ResolvedSpecbaseRoot,
      globalDataDir,
    });
  ...
  it('omits self-references silently, by id and by path', async () => {
    ...
    const byId = await assembleReferenceIndex({
      ...
      resolvedRoot: {
        ...
        changesDir: path.join(storeRoot, 'specbase', 'changes'),
        defaultSchema: 'spec-driven',
      } as ResolvedSpecbaseRoot,
      globalDataDir,
    });
  ...
  it('truncates at the 50KB budget with an order-preserving keep and a warning', async () => {
    ...
    expect(entry.status[0]).toEqual(
      expect.objectContaining({
        ...
        fix: expect.stringContaining('specbase list --specs --store huge-context'),
      })
    );
  ...
  it('renders the XML block and markdown section consistently', async () => {
    ...
    expect(block).toContain('Fetch: specbase show <spec-id> --type spec --store team-context');
    expect(block).toContain("Store missing-context: Referenced store 'missing-context' is not registered on this machine.");
    expect(block).toContain('Fix: Get a checkout from a teammate and run: specbase store register <path> --id missing-context');

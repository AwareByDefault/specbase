...
describe('PowerShellInstaller', () => {
  ...
  beforeEach(async () => {
    testHomeDir = path.join(os.tmpdir(), `specbase-powershell-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'SpecbaseCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'SpecbaseCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'SpecbaseCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/SpecbaseCompletion.ps1';

    // Note: SPECBASE_NO_AUTO_CONFIG check is now handled in the install() method,
    ...
    it.skipIf(process.platform === 'win32')('should not create profile directory when parent is not writable', async () => {
      const originalNoAutoConfig = process.env.SPECBASE_NO_AUTO_CONFIG;
      ...
      try {
        ...
        restoreEnvValue('SPECBASE_NO_AUTO_CONFIG', originalNoAutoConfig);
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(content).not.toContain('# SPECBASE:END');
      expect(content).not.toContain('# Specbase completions');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for Specbase
$specbaseCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName specbase -ScriptBlock $specbaseCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('SpecbaseCompletion.ps1');
    ...
    // Note: SPECBASE_NO_AUTO_CONFIG support was removed from PowerShell installer
    ...
    it('should handle installation with paths containing spaces', async () => {
      const spacedHomeDir = path.join(os.tmpdir(), `specbase powershell test ${randomUUID()}`);
      ...
      expect(result.success).toBe(true);
      expect(result.installedPath).toContain('specbase powershell test');
  ...
  describe('encoding preservation', () => {
    ...
    it('should preserve UTF-16 LE BOM when removing profile config', async () => {
      ...
      const textWithBlock = [
        '. "C:\\Code\\profile.ps1"',
        '# SPECBASE:START',
        '. "/path/to/SpecbaseCompletion.ps1"',
        '# SPECBASE:END',
        '',
      ].join('\n');
      ...
      // Verify content: original line kept, Specbase block removed
      const content = raw.subarray(2).toString('utf16le');
    ...
    it('should round-trip UTF-16 LE through install → uninstall without corruption', async () => {
      ...
      // Install adds the Specbase block
      ...
      // Uninstall removes the Specbase block
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# SPECBASE:START');
      ...
      expect(profileContentAfter).not.toContain('# SPECBASE:START');
    });

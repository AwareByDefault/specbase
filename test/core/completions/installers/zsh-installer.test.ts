...
describe('ZshInstaller', () => {
  ...
  beforeEach(async () => {
    ...
    testHomeDir = path.join(os.tmpdir(), `specbase-zsh-test-${randomUUID()}`);
  ...
  describe('getInstallationPath', () => {
    it('should return Oh My Zsh path when Oh My Zsh is installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.oh-my-zsh', 'custom', 'completions', '_specbase'));
    });
    ...
    it('should return standard Zsh path when Oh My Zsh is not installed', async () => {
      ...
      expect(result.path).toBe(path.join(testHomeDir, '.zsh', 'completions', '_specbase'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '#compdef specbase\n_specbase() {\n  echo "test"\n}\n';
    ...
    it('should install to Oh My Zsh path when Oh My Zsh is present', async () => {
      ...
      expect(result.isOhMyZsh).toBe(true);
      expect(result.installedPath).toBe(path.join(ohMyZshPath, 'custom', 'completions', '_specbase'));
    ...
    it('should install to standard Zsh path when Oh My Zsh is not present', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.isOhMyZsh).toBe(false);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.zsh', 'completions', '_specbase'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.zsh', 'completions', '_specbase');
    ...
    it('should include fpath instructions for standard Zsh when auto-config is disabled', async () => {
      const originalEnv = process.env.SPECBASE_NO_AUTO_CONFIG;
      ...
      if (originalEnv === undefined) {
        delete process.env.SPECBASE_NO_AUTO_CONFIG;
      } else {
        process.env.SPECBASE_NO_AUTO_CONFIG = originalEnv;
      }
    });
    ...
    it('should handle paths with spaces in .zshrc config', async () => {
      ...
      const testHomeDirWithSpaces = path.join(os.tmpdir(), `specbase zsh test ${randomUUID()}`);
  ...
  describe('uninstall', () => {
    ...
    it('should remove from correct location for Oh My Zsh', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(path.join('.oh-my-zsh', 'custom', 'completions', '_specbase'));
    });
  });
  ...
  describe('getInstallationInfo', () => {
    ...
    it('should return installation info when installed', async () => {
      ...
      expect(info.path).toContain('_specbase');
  ...
  describe('configureZshrc', () => {
    ...
    it('should create .zshrc with markers and config when file does not exist', async () => {
      ...
      expect(content).toContain('# SPECBASE:START');
      expect(content).toContain('# SPECBASE:END');
      expect(content).toContain('# Specbase shell completions configuration');
    ...
    it('should prepend markers and config when .zshrc exists without markers', async () => {
      ...
      const configIndex = content.indexOf('# SPECBASE:START');
    ...
    it('should preserve user content outside markers', async () => {
      ...
      expect(content).toContain(`fpath=("${completionsDir}" $fpath)`);
      expect(content).not.toContain('# Old Specbase config');
    });

    it('should return false when SPECBASE_NO_AUTO_CONFIG is set', async () => {
      const originalEnv = process.env.SPECBASE_NO_AUTO_CONFIG;
      process.env.SPECBASE_NO_AUTO_CONFIG = '1';
  ...
  describe('removeZshrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      expect(newContent).not.toContain('# SPECBASE:START');
      expect(newContent).not.toContain('# SPECBASE:END');
      expect(newContent).not.toContain('Specbase shell completions');
    ...
    it('should handle invalid marker placement gracefully', async () => {
      ...
      await fs.writeFile(zshrcPath, '# SPECBASE:END\n# SPECBASE:START\n');
    ...
    it('should return true when only one marker is present', async () => {
      const zshrcPath = path.join(testHomeDir, '.zshrc');
      await fs.writeFile(zshrcPath, '# SPECBASE:START\nsome config\n');
  ...
  describe('install with .zshrc auto-configuration', () => {
    ...
    it('should not configure .zshrc for Oh My Zsh', async () => {
      ...
      expect(content).not.toContain('# SPECBASE:START');
  ...
  describe('uninstall with .zshrc cleanup', () => {
    ...
    it('should remove .zshrc config when uninstalling', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain('Removed Specbase configuration from ~/.zshrc');

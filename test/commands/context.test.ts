...
describe('specbase context (4.1)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.realpathSync.native(fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-context-')));
    env = {
  ...
  it('assembles the working set from declarations, all session shapes', async () => {
    ...
    expect(workingSet.members).toEqual([
      {
        ...
        id: 'upstream-context',
        path: upstream,
        fetch: 'specbase show <spec-id> --type spec --store upstream-context',
        status: [],
      },
      {
        role: 'referenced_store',
    ...
    expect(human.stdout).toContain('Fetch: specbase show <spec-id> --type spec --store upstream-context');

...
describe('declared store fallback (3.2)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.realpathSync.native(
      fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-declared-'))
    );
    env = {
    ...
    fs.mkdirSync(path.join(pointerRepo, 'specbase'), { recursive: true });
    fs.writeFileSync(
      path.join(pointerRepo, 'specbase', 'config.yaml'),
      'store: team-context\n'
    );
  });
  ...
  it('runs the externalized-planning journey without --store anywhere', async () => {
    ...
    expect(statusHuman.stderr).toContain('Using Specbase root: team-context');
    ...
    const changeDir = path.join(storeRoot, 'specbase', 'changes', 'billing-rework');
    fs.writeFileSync(
    ...
    const archived = fs.readdirSync(path.join(storeRoot, 'specbase', 'changes', 'archive'));
  ...
  it('composes with 3.1: the declared root surfaces the store own references', async () => {
    ...
    createSpecbaseRoot(upstreamRoot);
  ...
  it('refuses init in a pointer repo and creates nothing, then converts cleanly', async () => {
    ...
    fs.writeFileSync(path.join(pointerRepo, 'specbase', 'config.yaml'), 'schema: spec-driven\n');
    const converted = await runCLI(['init', '.', '--tools', 'none'], {
      cwd: pointerRepo,
      env,
    });
    expect(converted.exitCode).toBe(0);
    expect(fs.existsSync(path.join(pointerRepo, 'specbase', 'specs'))).toBe(true);
    expect(fs.existsSync(path.join(pointerRepo, 'specbase', 'changes'))).toBe(true);
  });
  ...
  it('refuses init for malformed pointers and from pointer-repo subdirectories', async () => {
    ...
    expect(fs.existsSync(path.join(pointerRepo, 'specbase', 'specs'))).toBe(false);
    ...
    expect(fs.existsSync(path.join(subdir, 'specbase'))).toBe(false);
  });
  ...
  it('keeps real-root stdout byte-identical when a pointer is present, with one warning', async () => {
    ...
    for (const [label, config] of [
      ...
      fs.writeFileSync(path.join(realRepo, 'specbase', 'config.yaml'), config);

...
import { createHealthySpecbaseRoot, isolatedGitEnv } from '../helpers/store-git.js';
...
describe('store canonical remote (3.3)', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-store-remote-'));
    env = {
  ...
  describe('setup', () => {
    it('records --remote in store.yaml inside the initial commit', async () => {
      ...
      const committed = git(storeRoot, 'show', 'HEAD:.specbase-store/store.yaml');
      expect(committed).toContain(`remote: ${TEST_NET_URL}`);
      expect(committed).toBe(
        fs.readFileSync(path.join(storeRoot, '.specbase-store', 'store.yaml'), 'utf-8')
      );
    ...
    it('refuses --remote when store.yaml already exists, naming the hand-edit', async () => {
      ...
      const before = fs.readFileSync(path.join(storeRoot, '.specbase-store', 'store.yaml'), 'utf-8');
      ...
      expect(status[0].fix).toContain(path.join('.specbase-store', 'store.yaml'));
      expect(fs.readFileSync(path.join(storeRoot, '.specbase-store', 'store.yaml'), 'utf-8')).toBe(
        before
      );
    });
  ...
  describe('register', () => {
    function makeUnregisteredStore(name: string, options: { origin?: string; metadataRemote?: string } = {}): string {
      ...
      fs.mkdirSync(path.join(storeRoot, '.specbase-store'), { recursive: true });
      fs.writeFileSync(
        path.join(storeRoot, '.specbase-store', 'store.yaml'),
        `version: 1\nid: ${name}\n` +
  ...
  describe('onboarding end to end', () => {
    it('executes the printed clone fix verbatim and continues to a resolved index', async () => {
      ...
      createHealthySpecbaseRoot(originWorktree);
      // Anchor every directory a healthy clone needs (the same job
      // store setup's anchor files do).
      fs.writeFileSync(path.join(originWorktree, 'specbase', 'specs', '.gitkeep'), '');
      fs.writeFileSync(path.join(originWorktree, 'specbase', 'changes', 'archive', '.gitkeep'), '');
      fs.mkdirSync(path.join(originWorktree, '.specbase-store'), { recursive: true });
      fs.writeFileSync(
        path.join(originWorktree, '.specbase-store', 'store.yaml'),
        'version: 1\nid: team-context\n'
      );
      ...
      fs.mkdirSync(path.join(appRepo, 'specbase'), { recursive: true });
      fs.writeFileSync(
        path.join(appRepo, 'specbase', 'config.yaml'),
        'schema: spec-driven\nreferences:\n' +
          `  - { id: team-context, remote: ${originRemote} }\n`
      );
      fs.mkdirSync(path.join(appRepo, 'specbase', 'specs'), { recursive: true });
      fs.mkdirSync(path.join(appRepo, 'specbase', 'changes', 'archive'), { recursive: true });
      ...
      expect(fix).toBe(
        `git clone -- ${originRemote} ${q}${expectedCheckout}${q} && specbase store register ${q}${expectedCheckout}${q} --id team-context`
      );
  ...
  describe('doctor and resolution', () => {
    it('surfaces both remotes, prefers canonical in human output, no new diagnostics', async () => {
      ...
      fs.writeFileSync(path.join(storeRoot, 'specbase', 'specs', '.gitkeep'), '');
      fs.writeFileSync(path.join(storeRoot, 'specbase', 'changes', 'archive', '.gitkeep'), '');
      fs.mkdirSync(path.join(storeRoot, '.specbase-store'), { recursive: true });
      fs.writeFileSync(
        path.join(storeRoot, '.specbase-store', 'store.yaml'),
        `version: 1\nid: doc-context\nremote: ${canonical}\n`

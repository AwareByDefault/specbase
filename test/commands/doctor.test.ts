...
describe('specbase doctor (3.6)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.realpathSync.native(fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-doctor-')));
    env = {
  ...
  it('reports ok everywhere for a healthy store-backed root, all session shapes', async () => {
    ...
    expect(human.stderr).toContain('Using Specbase root: team-context');
    ...
    fs.writeFileSync(path.join(pointerRepo, 'specbase', 'config.yaml'), 'store: team-context\n');
  ...
  it('prints taxonomy errors in human mode instead of stack traces', async () => {
    ...
    expect(result.stderr).toContain('Error: No Specbase root found');

...
describe('store references in instructions (3.1)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-store-refs-'));
    env = {
    ...
    expect(payload.references).toEqual([
      {
        ...
        fetch: 'specbase show <spec-id> --type spec --store team-context',
        status: [],
      },
    ]);
  ...
  it('omits the references field entirely when none are declared', async () => {
    fs.writeFileSync(path.join(appRepo, 'specbase', 'config.yaml'), 'schema: spec-driven\n');
  ...
  it('completes the PM-to-dev layered flow end to end', async () => {
    ...
    const changeDir = path.join(appRepo, 'specbase', 'changes', 'billing-rework');
    fs.writeFileSync(

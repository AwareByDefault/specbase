...
describe('artifact-workflow CLI commands', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'specbase-artifact-workflow-'));
    changesDir = path.join(tempDir, 'specbase', 'changes');
  ...
  describe('status command', () => {
    ...
    it('exits gracefully when no changes exist', async () => {
      ...
      expect(result.stdout).toContain('specbase new change');
    });
  ...
  describe('instructions apply command', () => {
    ...
    it('resolves single-star glob artifacts consistently between status and apply', async () => {
      const schemaDir = path.join(tempDir, 'specbase', 'schemas', 'glob-test');
...
`
    ...
    it('fallback: requires all artifacts when schema has no apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'specbase', 'schemas', 'no-apply');
...
`;
    ...
    it('fallback: ready when all artifacts exist for schema without apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'specbase', 'schemas', 'no-apply-full');
      const templatesDir = path.join(noApplySchemaDir, 'templates');
      await fs.mkdir(templatesDir, { recursive: true });
...
`;
  ...
  describe('experimental command (deprecated alias for init)', () => {
    ...
    it('creates skills for Claude tool', async () => {
      ...
      const skillFile = path.join(tempDir, '.claude', 'skills', 'specbase-explore', 'SKILL.md');
    ...
    it('creates skills for Cursor tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.cursor', 'skills', 'specbase-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);

      // Verify commands were created with Cursor format
      const commandFile = path.join(tempDir, '.cursor', 'commands', 'spcb-explore.md');
    ...
    it('creates skills for Windsurf tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.windsurf', 'skills', 'specbase-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    });
  });
  ...
  describe('project config integration', () => {
    describe('new change uses config schema', () => {
      it('creates change with schema from project config', async () => {
        ...
        // Note: changesDir is already at tempDir/specbase/changes (created in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'specbase', 'config.yaml'),
          'schema: spec-driven\n'
        );
      ...
      it('CLI schema overrides config schema', async () => {
        // Create project config with spec-driven schema
        // Note: specbase directory already exists (from changesDir creation in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'specbase', 'config.yaml'),
          'schema: spec-driven\n'
        );

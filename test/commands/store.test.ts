...
import { createHealthySpecbaseRoot } from '../helpers/store-git.js';
...
async function runStoreCommand(args: string[]): Promise<void> {
  ...
  await program.parseAsync(['node', 'specbase', 'store', ...args]);
}
...
describe('store command', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-store-command-'));
  ...
  function expectHealthySpecbaseRoot(root: string): void {
  ...
  it('sets up a store at an explicit path without Git in non-interactive JSON mode', async () => {
    ...
    expectHealthySpecbaseRoot(storeRoot);
  ...
  it('runs guided setup when no args are passed in an interactive terminal', async () => {
    ...
    expect(input).toHaveBeenCalledWith(expect.objectContaining({
      ...
      default: '~/specbase/guided-context',
    }));
  ...
  it('preserves an existing healthy Specbase root during setup', async () => {
    const storeRoot = mkdir('team-context');
    createHealthySpecbaseRoot(storeRoot, 'config.yml');
    fs.writeFileSync(path.join(storeRoot, 'specbase', 'specs', 'note.md'), 'keep\n');
    ...
    expect(fs.existsSync(path.join(storeRoot, 'specbase', 'config.yaml'))).toBe(false);
    expect(fs.readFileSync(path.join(storeRoot, 'specbase', 'config.yml'), 'utf-8')).toBe(
      `schema: ${DEFAULT_SPECBASE_SCHEMA}\n`
    );
    expect(fs.readFileSync(path.join(storeRoot, 'specbase', 'specs', 'note.md'), 'utf-8')).toBe('keep\n');
  });

  it('ignores old beta files inside an otherwise healthy root', async () => {
    const storeRoot = mkdir('team-context');
    createHealthySpecbaseRoot(storeRoot);
  ...
  it('refuses to convert a config-only store pointer repo into a store', async () => {
    ...
    fs.mkdirSync(path.join(pointerRoot, 'specbase'), { recursive: true });
    fs.writeFileSync(path.join(pointerRoot, 'specbase', 'config.yaml'), 'store: team-context\n');
    ...
    expect(fs.existsSync(path.join(pointerRoot, 'specbase', 'specs'))).toBe(false);
    expect(fs.existsSync(path.join(pointerRoot, 'specbase', 'changes'))).toBe(false);
  ...
  it('registers a team store before any changes exist', async () => {
    const storeRoot = mkdir('team-context');
    fs.mkdirSync(path.join(storeRoot, 'specbase'), { recursive: true });
    fs.writeFileSync(
    ...
    expect(payload.store.id).toBe('team-context');
    expect(payload.created_files).toEqual([]);
    expect(fs.existsSync(path.join(storeRoot, 'specbase', 'changes'))).toBe(false);
    expect(fs.existsSync(path.join(storeRoot, 'specbase', 'specs'))).toBe(false);
    expect(fs.existsSync(path.join(storeRoot, 'specbase', 'changes', 'archive'))).toBe(false);
  });

  it('registers a store with active changes before specs or archive exist', async () => {
    const storeRoot = mkdir('team-context');
    fs.mkdirSync(path.join(storeRoot, 'specbase', 'changes', 'add-widget'), { recursive: true });
    fs.writeFileSync(
      path.join(storeRoot, 'specbase', 'changes', 'add-widget', 'proposal.md'),
      '# Proposal\n'
    );
    fs.writeFileSync(
      path.join(storeRoot, 'specbase', 'config.yaml'),
  ...
  it('requires confirmation before registering a healthy root without identity', async () => {
    ...
    expect(parseJson(confirmed).created_files).toEqual(['.specbase-store/store.yaml']);
    await expect(readStoreMetadataState(storeRoot)).resolves.toEqual({
      version: 1,
      id: 'team-context',
    });
  ...
  it('writes nothing when interactive register conversion is declined', async () => {
    ...
    expect(confirm).toHaveBeenCalledWith({
      message: "Turn this Specbase root into store 'team-context'?",
      default: false,
    });
    expect(fs.existsSync(getStoreMetadataPath(storeRoot))).toBe(false);
  ...
  it('reports repeated setup and register as no-op success', async () => {
    const storeRoot = mkdir('team-context');
    createHealthySpecbaseRoot(storeRoot);
    fs.writeFileSync(path.join(storeRoot, 'specbase', 'config.yaml'), 'schema: spec-driven\n# user edit\n');
    ...
    expect(fs.readFileSync(path.join(storeRoot, 'specbase', 'config.yaml'), 'utf-8')).toBe(
      'schema: spec-driven\n# user edit\n'
    );
    await expect(readStoreRegistryState({ globalDataDir })).resolves.toEqual({
      version: 1,
  ...
  it('rejects registry id and alias path conflicts', async () => {
    ...
    createHealthySpecbaseRoot(firstRoot);
    createHealthySpecbaseRoot(secondRoot);
    ...
    fs.rmSync(path.join(firstRoot, '.specbase-store'), { recursive: true, force: true });
  ...
  it('doctors registered store path, metadata, and Git presence', async () => {
    ...
    createHealthySpecbaseRoot(healthyRoot);
    createHealthySpecbaseRoot(mismatchRoot);
    ...
    expect(byId['healthy-context'].specbase_root.healthy).toBe(true);
    ...
    expect(byId['missing-context'].specbase_root.present).toBeNull();
  ...
  it('reports Specbase root health separately without repairing it', async () => {
    const storeRoot = mkdir('team-context');
    fs.mkdirSync(path.join(storeRoot, 'specbase', 'specs'), { recursive: true });
    fs.mkdirSync(path.join(storeRoot, 'specbase', 'changes'), { recursive: true });
    fs.writeFileSync(path.join(storeRoot, 'specbase', 'changes', 'archive'), 'not a dir\n');
    fs.writeFileSync(path.join(storeRoot, 'specbase', 'config.yaml'), `schema: ${DEFAULT_SPECBASE_SCHEMA}\n`);
    await writeStoreMetadataState(storeRoot, { version: 1, id: 'team-context' });
    await writeStoreRegistryState(
      {
        version: 1,
    ...
    expect(store.specbase_root.archive.present).toBe(false);
    expect(store.specbase_root.status[0]).toEqual(
      expect.objectContaining({
        code: 'specbase_archive_not_directory',
      })
    );
    expect(fs.readFileSync(path.join(storeRoot, 'specbase', 'changes', 'archive'), 'utf-8')).toBe('not a dir\n');
  });
  ...
  it('register errors are terminal: one-checkout rule, no circular fix texts', async () => {
    ...
    createHealthySpecbaseRoot(original);
    ...
    createHealthySpecbaseRoot(secondCheckout);
    ...
    expect(conflictStatus.fix).toContain('specbase store unregister team-context');
    ...
    createHealthySpecbaseRoot(freeRoot);
  ...
  describe('committed format and data dir guards', () => {
    ...
    it('pins the committed store metadata literals and the stores data dir', async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(fs.existsSync(path.join(storeRoot, '.specbase-store', 'store.yaml'))).toBe(true);
    ...
    it('registers a store repo created before the rename', async () => {
      ...
      createHealthySpecbaseRoot(storeRoot);
      const metadataDir = path.join(storeRoot, '.specbase-store');
  ...
  describe('store group surface', () => {
    it('hints lifecycle attempts under the store group at --store', async () => {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toContain("unknown command 'new' for 'specbase store'");
      expect(result.stderr).toContain(
        'setup, register, unregister, remove, list (ls), doctor'
      );
      expect(result.stderr).toContain('specbase new change billing-rework --store <id>');
    });
    ...
    it('never suggests an invalid command for partial new invocations', async () => {
      ...
      expect(result.stderr).toContain('specbase new change <change-id> --store <id>');
      expect(result.stderr).not.toContain('specbase new my-change');
    });

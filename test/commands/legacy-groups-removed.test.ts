...
describe('legacy command groups are removed', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'specbase-legacy-removed-'));
    env = {
  ...
  function writeWorkspaceViewFixture(dir: string): void {
    const metadataDir = path.join(dir, '.specbase-workspace');
  ...
  it('update falls through to the standard no-project error in a view dir', async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain('No Specbase directory found');
  ...
  it('keeps initiative data and view state byte-identical across surviving commands', async () => {
    ...
    createHealthySpecbaseRoot(otherRoot);
    ...
    const viewBefore = snapshotDirectory(path.join(projectDir, '.specbase-workspace'));
    ...
    expect(snapshotDirectory(path.join(projectDir, '.specbase-workspace'))).toEqual(viewBefore);
  ...
  it('tolerates legacy initiative metadata without re-emitting it', async () => {
    ...
    createHealthySpecbaseRoot(projectDir);
    const changeDir = path.join(projectDir, 'specbase', 'changes', 'legacy-change');
  ...
  it('reports repo-local in a view dir, exactly as before this slice', async () => {
    ...
    createHealthySpecbaseRoot(projectDir);
    writeWorkspaceViewFixture(projectDir);
    const changeDir = path.join(projectDir, 'specbase', 'changes', 'mode-check');
    fs.mkdirSync(changeDir, { recursive: true });

...
describe('FileSystemUtils.updateFileWithMarkers', () => {
  let testDir: string;
  const START_MARKER = '<!-- SPECBASE:START -->';
  const END_MARKER = '<!-- SPECBASE:END -->';

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `specbase-marker-test-${Date.now()}`);
  ...
  describe('new file creation', () => {
    it('should create new file with markers and content', async () => {
      ...
      const content = 'Specbase content';
  ...
  describe('existing file without markers', () => {
    it('should prepend markers and content to existing file', async () => {
      ...
      const newContent = 'Specbase content';
      await FileSystemUtils.updateFileWithMarkers(
        filePath,
        newContent,
        START_MARKER,
  ...
  describe('existing file with markers', () => {
    it('should replace content between markers', async () => {
      ...
      const oldManagedContent = 'Old Specbase content';
      const afterContent = '# After\nSome content after';
      
      const existingFile = `${beforeContent}\n${START_MARKER}\n${oldManagedContent}\n${END_MARKER}\n${afterContent}`;
      await fs.writeFile(filePath, existingFile);
      
      const newContent = 'New Specbase content';
      await FileSystemUtils.updateFileWithMarkers(
        filePath,
        newContent,
        START_MARKER,
...
After block content`;
  ...
  describe('shell markers', () => {
    const SHELL_START = '# SPECBASE:START';
    const SHELL_END = '# SPECBASE:END';
...
export EDITOR="vim"`;
      ...
      expect(result).not.toContain('alias specbase');

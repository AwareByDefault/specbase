...
function main() {
  ...
  try {
    log(`Packing @awarebydefault/specbase@${expected}...`);
    const filename = npmPack();
    tgzPath = path.resolve(filename);
    log(`Created: ${tgzPath}`);

    work = mkdtempSync(path.join(tmpdir(), 'specbase-pack-check-'));
    log(`Temp dir: ${work}`);
    ...
    const binRel = path.join('node_modules', '@awarebydefault', 'specbase', 'bin', 'specbase.js');

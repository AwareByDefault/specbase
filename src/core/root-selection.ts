/**
 * Shared Specbase root resolution for normal commands.
 *
 * Normal commands (`new change`, `status`, `instructions`, `list`, `show`,
 * `validate`, `archive`) resolve one Specbase root through this module:
 *
 * - `--store <id>` selects a registered store's root.
 * - Without `--store`, the nearest ancestor containing `specbase/` wins.
...
import { inspectSpecbaseRoot } from './openspec-root.js';
...
export type SpecbaseRootSource = 'store' | 'declared' | 'nearest' | 'implicit';
...
export interface ResolveSpecbaseRootOptions extends StoreSelectorOptions {
  startPath?: string;
  allowImplicitRoot?: boolean;
  globalDataDir?: string;
}

export interface ResolvedSpecbaseRoot {
  path: string;
  changesDir: string;
  specsDir: string;
  archiveDir: string;
  defaultSchema: 'spec-driven';
  source: SpecbaseRootSource;
  storeId?: string;
}
...
function doctorFix(id: string): string {
  return `Run specbase store doctor ${id} to inspect it.`;
}
...
async function resolveStoreRoot(
  id: string,
  globalDataDir?: string,
  source: SpecbaseRootSource = 'store'
): Promise<ResolvedSpecbaseRoot> {
  try {
    validateStoreId(id);
  } catch (error) {
  ...
  if (!entry) {
    if (entries.length === 0) {
      throw new RootSelectionError(
        ...
        {
          target: 'store.id',
          fix: `Run specbase store setup ${id} or specbase store register <path> first.`,
        }
      );
    }
    ...
    throw new RootSelectionError(
      ...
      {
        target: 'store.id',
        fix: 'Pass a registered store id, or run specbase store list.',
      }
    );
  }
  ...
  switch (inspection.kind) {
    ...
    case 'unhealthy_root':
      throw new RootSelectionError(
        `Store '${id}' does not have a healthy Specbase root at ${storeRoot}: ${inspection.problems} ${doctorFix(id)}`,
        'unhealthy_store_root',
        { target: 'specbase.root', fix: doctorFix(id) }
      );
    case 'ok':
...
export async function inspectRegisteredStore(
  ...
  const inspection = await inspectSpecbaseRoot(storeRoot);
  if (!inspection.healthy) {
    const problems =
      inspection.diagnostics.map((diagnostic) => diagnostic.message).join(' ') ||
      'Specbase root is missing or incomplete.';
...
/**
 * Classifies the nearest `specbase/` directory (slice 3.2): a planning
 ...
 */
/**
 * The nearest-root walk, qualified: an `specbase/` DIRECTORY alone is
 * not a root — it must carry a planning shape or a config file.
 * Without this, the recommended `~/specbase/<id>` store layout would
...
function findQualifyingRootSync(startPath: string): string | null {
  ...
  while (candidate) {
    const { hasPlanningShape, pointer } = classifySpecbaseDir(candidate);
...
async function resolveNearestOrDeclaredRoot(
  ...
  const { hasPlanningShape, pointer } = classifySpecbaseDir(nearestRoot);

  if (hasPlanningShape) {
    if (pointer.value !== undefined) {
      console.error(
        `Warning: ${pointer.filePath} declares store '${pointer.value}', but this directory is a real Specbase root; the declaration is ignored.`
      );
    }
    ...
    if (error instanceof RootSelectionError) {
      ...
      const declarationFix =
        error.diagnostic.code === 'unknown_store'
          ? `Register the store (specbase store register <path> --id ${pointer.value}) or edit ${pointer.filePath} to name a registered store.`
...
export async function resolveSpecbaseRoot(
  options: ResolveSpecbaseRootOptions = {}
): Promise<ResolvedSpecbaseRoot> {
  if (options.storePath !== undefined) {
    throw new RootSelectionError(
      '--store-path is not supported. Register the path with specbase store register <path>, then select it with --store <id>.',
      'store_path_not_supported',
      {
        target: 'store.id',
        fix: 'specbase store register <path>, then rerun with --store <id>.',
      }
    );
  }
  ...
  if (registeredIds.length > 0) {
    throw new RootSelectionError(
      `No Specbase root found in the current directory or its ancestors. Registered stores: ${registeredIds.join(', ')}. Pass --store <id> to use one, or run specbase init to create a local root.`,
      'no_root_with_registered_stores',
      {
        target: 'specbase.root',
        fix: `Rerun with --store <id> (registered: ${registeredIds.join(', ')}) or run specbase init.`,
      }
    );
  }
  ...
  if (options.allowImplicitRoot === false) {
    throw new RootSelectionError(
      'No Specbase root found from the current directory.',
      'no_specbase_root',
      { target: 'specbase.root', fix: 'Run specbase init to create a root here.' }
    );
  }
...
export function toRootOutput(root: ResolvedSpecbaseRoot): RootOutput {
  return {
    path: root.path,
    source: root.source,
...
): root is ResolvedSpecbaseRoot & { storeId: string } {
 ...
 */
export function emitStoreRootBanner(root: ResolvedSpecbaseRoot): void {
  if (isStoreSelectedRoot(root)) {
    console.error(`Using Specbase root: ${root.storeId} (${root.path})`);
  }
}
 ...
 */
export function withStoreFlag(root: ResolvedSpecbaseRoot, command: string): string {
 ...
 */
export function toPlanningHome(root: ResolvedSpecbaseRoot): PlanningHome {
  return {
    kind: 'repo',
    root: root.path,

  ...
  for (const entry of listStoreRegistryEntries(registry ?? { version: 1, stores: {} })) {
    ...
    if (entry.id === id) {
      throw new StoreError(
        ...
        {
          target: 'store.id',
          fix: `Use the existing registration, or run specbase store unregister ${id} first to switch this id to a different checkout.`,
        }
      );
    }
    ...
    if (!options.writeIfMissing) {
      throw new StoreError(
        ...
        'store_metadata_missing',
        {
          ...
          fix: `Create ${getStoreMetadataPath(storeRoot)} or rerun "specbase store register <path>".`,
        }
      );
    }
...
export async function resolveRegisteredStore(
  ...
  if (!registry) {
    throw new StoreError('No store registry found', 'no_store_registry', {
      target: 'store.id',
      fix: 'Register a store with specbase store register <path>, then select it with --store <id>.',
    });
  }

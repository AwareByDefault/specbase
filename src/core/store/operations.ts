...
export interface StoreInspection extends StoreInfo {
  specbaseRoot: SpecbaseRootInspection;
  metadata: {
...
function assertNotConfigOnlyPointerRoot(storeRoot: string): void {
  const { hasPlanningShape, pointer } = classifySpecbaseDir(storeRoot);
...
function resolveSetupRoot(id: string, inputPath: string | undefined): string {
  ...
  if (inputPath === undefined || inputPath.trim().length === 0) {
    throw new StoreError(
      ...
      {
        target: 'store.root',
        fix: `specbase store setup ${id} --path ~/specbase/${id}`,
      }
    );
  }
...
function resolveRegisterRoot(inputPath: string | undefined): string {
  if (inputPath === undefined || inputPath.trim().length === 0) {
    throw new StoreError('Pass a store path.', 'store_path_required', {
      target: 'store.root',
      fix: 'specbase store register /path/to/store',
    });
  }
...
async function prepareSetupPlan(
  ...
  if (kind === 'file' || kind === 'other') {
    throw new StoreError(
      ...
      {
        target: 'store.root',
        fix: 'Choose an empty directory or an existing healthy Specbase root.',
      }
    );
  }
  ...
  if (kind === 'directory') {
    ...
    if (metadata) {
      ...
      const specbaseRoot = await inspectSpecbaseRoot(storeRoot);
      const safeFreshDirectory = await isDirectoryEmpty(storeRoot) || await isGitOnlyDirectory(storeRoot);
      if (!specbaseRoot.healthy && !safeFreshDirectory) {
        throw new StoreError(
          'Store setup does not support initializing a non-empty folder that is not a healthy Specbase root.',
          'store_setup_non_empty_directory',
          {
            target: 'store.root',
            fix: 'Choose an empty folder, a Git-only folder, or an existing healthy Specbase root.',
          }
        );
      }
    }
...
export async function setupPreparedStore(
  ...
  if (kind === 'missing' && (await fs.access(storeRoot).then(() => true, () => false))) {
    throw new StoreError(
      ...
      {
        target: 'store.root',
        fix: 'Rerun specbase store setup to re-evaluate the directory.',
      }
    );
  }
  ...
  try {
    const root = await ensureSpecbaseRoot(storeRoot, {
    ...
    if (!existingMetadata) {
      ...
      if (metadataDirMissing) {
        createdPaths.push(createdPath('.specbase-store/', metadataDir, 'directory'));
      }
      ...
      createdFiles.push('.specbase-store/store.yaml');
    }
...
export async function registerExistingStore(
  ...
  assertNotConfigOnlyPointerRoot(storeRoot);
  const specbaseRoot = await inspectSpecbaseRoot(storeRoot);
  if (!specbaseRoot.healthy) {
    const problems =
      specbaseRoot.diagnostics.map((diagnostic) => diagnostic.message).join(' ') ||
      'The Specbase root is missing or incomplete.';
    ...
    throw new StoreError(
      `Store register requires an existing healthy Specbase root. ${problems}${emptyCloneHint}`,
      'store_register_root_unhealthy',
      {
        ...
        fix: isEmptyCloneSuspect
          ...
          : 'Run specbase store setup for a new store, or point register at a checkout whose specbase/ files are present.',
      }
    );
  }
  ...
  if (metadata && explicitId !== undefined && metadata.id !== explicitId) {
    ...
    throw new StoreError(
      `Store metadata id '${metadata.id}' does not match --id '${explicitId}'. The id comes from the store's committed .specbase-store/store.yaml.`,
      'store_metadata_id_mismatch',
      {
        target: 'store.id',
        fix: registeredElsewhere
          ? `One checkout per store id is supported, and '${metadata.id}' is already registered. Run specbase store unregister ${metadata.id} first to register this checkout instead.`
  ...
  if (!metadata && !input.allowCreateIdentity) {
    throw new StoreError(
      `Turn this Specbase root into store '${id}'?`,
...
async function assertSafeToDeleteStoreRoot(storeRoot: string, id: string): Promise<{
  ...
  if (kind !== 'directory') {
    throw new StoreError(
      ...
      {
        target: 'store.root',
        fix: 'Run "specbase store unregister <id>" if you only want to forget this local registry entry.',
      }
    );
  }
...
async function inspectStore(entry: {
  ...
  let specbaseRoot: SpecbaseRootInspection = await inspectSpecbaseRoot(root);
  ...
  if (kind === 'missing') {
    diagnostics.push(makeStoreDiagnostic(
      ...
      {
        target: 'store.root',
        fix: `Run specbase store register /path/to/${entry.id} --id ${entry.id}.`,
      }
    ));
    ...
    specbaseRoot = await inspectSpecbaseRoot(root);
    diagnostics.push(...specbaseRoot.diagnostics);
    ...
    if (isRepository) {
      ...
      if (git.hasCommits === false) {
        ...
        for (const relativeDir of ANCHORED_SPECBASE_DIRS) {
  ...
  if (!registry) {
    if (selectedId !== undefined) {
      throw new StoreError(`Unknown store '${selectedId}'.`, 'store_not_found', {
        target: 'store.id',
        fix: 'Run specbase store list to see registered stores.',
      });
    }

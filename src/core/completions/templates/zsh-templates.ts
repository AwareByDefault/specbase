...
# Use specbase __complete to get available changes
_specbase_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(specbase __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use specbase __complete to get available specs
_specbase_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(specbase __complete specs 2>/dev/null)
...
_specbase_complete_schemas() {
  ...
  while IFS=$'\\t' read -r id desc; do
    schemas+=("$id:$desc")
  done < <(specbase __complete schemas 2>/dev/null)

...
/**
 * Generates PowerShell completion scripts for the Specbase CLI.
...
export class PowerShellGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# PowerShell completion script for Specbase CLI
...
`;
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}Get-SpecbaseChanges | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);
        lines.push(`${indent}    [System.Management.Automation.CompletionResult]::new($_, $_, "ParameterValue", "Change: $_")`);
        lines.push(`${indent}}`);
        break;
      case 'spec-id':
        lines.push(`${indent}Get-SpecbaseSpecs | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);
      ...
      case 'work-item-id':
        ...
        break;
      case 'schema-name':
        lines.push(`${indent}Get-SpecbaseSchemas | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`);

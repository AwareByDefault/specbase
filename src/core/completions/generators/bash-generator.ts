...
/**
 * Generates Bash completion scripts for the Specbase CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for Specbase CLI
...
${helpers}
complete -F _specbase_completion specbase
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_specbase_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_specbase_complete_specs`);
        break;

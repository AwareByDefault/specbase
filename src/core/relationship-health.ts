...
import { toRootOutput, type ResolvedSpecbaseRoot } from './root-selection.js';

export interface RelationshipHealth {
  root: {
    path: string;
    source: ResolvedSpecbaseRoot['source'];
    store_id?: string;
    healthy: boolean;
  ...
  if (input.bothShapesPointer) {
    status.push(
      warning(
        ...
        `${input.bothShapesPointer.filePath} declares store '${input.bothShapesPointer.value}', but this directory is a real Specbase root; the declaration is ignored.`,
  ...
  if (input.inertPointerDeclarations && input.inertPointerDeclarations.fields.length > 0) {
    status.push(
      warning(
        ...
        `Move the ${input.inertPointerDeclarations.fields.join('/')} declarations into the store's specbase/config.yaml.`
      )
    );
  }

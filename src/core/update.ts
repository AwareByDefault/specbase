 ...
 * Update Command
 *
 * Refreshes Specbase skills and commands for configured tools.
...
import { AI_TOOLS, resolvePlanningDirName } from './config.js';
import {
  generateCommands,
...
export class UpdateCommand {
  ...
  async execute(projectPath: string): Promise<void> {
    ...
    const specbasePath = path.join(resolvedProjectPath, resolvePlanningDirName(resolvedProjectPath));

    // 1. Check the planning directory exists
    if (!await FileSystemUtils.directoryExists(specbasePath)) {
      throw new Error(`No Specbase directory found. Run 'specbase init' first.`);
    }

    // 2. Perform one-time migration if needed before any legacy upgrade generation.
    // Use detected tool directories to preserve existing spcb skills/commands.
    ...
    if (configuredTools.length === 0 && newlyConfiguredTools.length === 0) {
      ...
      console.log(chalk.dim('Run "specbase init" to set up tools.'));
      return;
    }
    ...
    const toolStatuses = configuredTools.map((toolId) => {
      const status = getToolVersionStatus(resolvedProjectPath, toolId, SPECBASE_VERSION);
    ...
    if (updatedTools.length > 0) {
      console.log(chalk.green(`✓ Updated: ${updatedTools.join(', ')} (v${SPECBASE_VERSION})`));
    }
    ...
    if (newlyConfiguredTools.length > 0) {
      ...
      console.log('  /spcb:new       Start a new change');
      console.log('  /spcb:continue  Create the next artifact');
      console.log('  /spcb:apply     Implement tasks');
      console.log();
  ...
  private displayUpToDateMessage(toolStatuses: ToolVersionStatus[]): void {
    ...
    console.log(chalk.green(`✓ All ${toolStatuses.length} tool(s) up to date (v${SPECBASE_VERSION})`));
  ...
  private displayUpdatePlan(
    ...
    const updates = toolsToUpdate.map((toolId) => {
      ...
      if (status?.needsUpdate) {
        ...
        return `${status.toolId} (${fromVersion} → ${SPECBASE_VERSION})`;
      }
  ...
  private detectNewTools(projectPath: string, configuredTools: string[]): void {
    ...
    if (newTools.length > 0) {
      ...
      console.log(
        chalk.yellow(
          `Detected new ${toolNoun}: ${newToolNames.join(', ')}. Run 'specbase init' to add ${pronoun}.`
        )
      );
    }
  }
  ...
  private displayExtraWorkflowsNote(
    ...
    if (extraWorkflows.length > 0) {
      console.log(chalk.dim(`Note: ${extraWorkflows.length} extra workflows not in profile (use \`specbase config profile\` to manage)`));
    }
  }
  ...
  private displayOldCoreCustomProfileNote(profile: Profile, workflows?: readonly string[]): void {
    ...
    console.log(chalk.dim('Run `specbase config profile core` and then `specbase update` to add sync.'));
  }
  ...
  /**
   * Detect and handle legacy Specbase artifacts.

/**
 * Review-panel orchestration workflow (add-review-panel-enforcement, spec
 * `spec-review-panel`). GOVERNED-ONLY: this template is registered in
 * skill-generation only when the resolved spec model is governed, so legacy
 * projects never receive it and legacy output stays byte-identical.
 *
 * The panel is the Kairos review-panel generalized onto the governed model. It
 * ORCHESTRATES blind per-lens reviewers; it never reviews the diff itself and it
 * never gates. Its findings are `review`-strength evidence, weaker than
 * automated proof by construction. Policy is sliced fresh from the governed
 * specs each lens's scope covers at review time — never copied into a lens
 * method — so a spec edit changes the review with no lens edit.
 *
 * Skill and command projections share one body constant (parity requirement):
 * `getReviewPanelSkillTemplate` and `getReviewPanelCommandTemplate` emit the same
 * pipeline, so the two surfaces cannot disagree.
 */
import type { SkillTemplate, CommandTemplate } from '../types.js';

/**
 * The shared review-panel body. Both projections interpolate this verbatim so
 * skill == command. It encodes the full pipeline plus the four default lens
 * methods and the lens-scope routing rule.
 */
const REVIEW_PANEL_BODY = `Run the governed review panel over a change: a panel of narrow, blind, per-lens
...
const REVIEW_PANEL_DESCRIPTION =
  'Run the governed review panel — router, deterministic gate, blind per-lens reviewers over the residue, refute-verify, completeness critic, read-only severity/lens report. Governed-model review analog of the lint config; findings are review-strength and never gate.';

/**
 * The review-panel SKILL projection. Governed-only; the body is identical to the
 * command projection (parity). Ignores `specModel` for content — the registry
 * only registers it under the governed model.
 */
export function getReviewPanelSkillTemplate(): SkillTemplate {
  return {
    name: 'openspec-review-panel',
    description: REVIEW_PANEL_DESCRIPTION,
    instructions: REVIEW_PANEL_BODY,
    license: 'MIT',

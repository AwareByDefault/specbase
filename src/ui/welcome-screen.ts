...
function getWelcomeText(): string[] {
  return [
    ...
    chalk.dim('  • /spcb:* slash commands'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('/spcb:new')}      ${chalk.dim('Create a change')}`,
    `  ${chalk.yellow('/spcb:continue')} ${chalk.dim('Next artifact')}`,
    `  ${chalk.yellow('/spcb:apply')}    ${chalk.dim('Implement tasks')}`,
    '',

...
type SpecbaseRootOutput = Omit<StoreInspection['specbaseRoot'], 'diagnostics'> & {
  status: StoreDiagnostic[];
};

interface StoreDoctorStoreOutput extends StoreOutput {
  specbase_root: SpecbaseRootOutput;
...
function toSpecbaseRootOutput(root: StoreInspection['specbaseRoot']): SpecbaseRootOutput {
  return {
...
function toDoctorStoreOutput(store: StoreInspection): StoreDoctorStoreOutput {
  return {
    ...toStoreOutput(store),
    specbase_root: toSpecbaseRootOutput(store.specbaseRoot),
...
async function promptStorePath(id: string): Promise<string> {
  ...
  const defaultPath = ['~', 'specbase', id].join('/');
...
async function resolveSetupInput(
  ...
  if (!id && !interactive) {
    throw new StoreError(
      ...
      {
        target: 'store.id',
        fix: 'specbase store setup <id> --path ~/specbase/<id> --json',
      }
    );
  }
  ...
  if (options.path === undefined && !interactive) {
    throw new StoreError(
      ...
      {
        target: 'store.root',
        fix: `specbase store setup ${id ?? '<id>'} --path ~/specbase/${id ?? '<id>'}`,
      }
    );
  }
...
async function confirmSetup(
  ...
  console.log('');
  console.log('Specbase will create:');
  console.log('');
...
async function confirmRemove(id: string, root: string, options: StoreRemoveOptions): Promise<void> {
  ...
  if (options.json || !isInteractive()) {
    throw new StoreError(
      ...
      {
        target: 'store.root',
        fix: `specbase store remove ${id} --yes`,
      }
    );
  }
  ...
  if (!confirmed) {
    throw new StoreError(
      ...
      {
        target: 'store.root',
        fix: 'Run "specbase store unregister <id>" if you only want to forget the local registration.',
      }
    );
  }
}
...
function printMutationHuman(
  ...
  console.log('Specbase root: ready');
  ...
  console.log('');
  console.log('Next: run normal Specbase commands against this store, for example:');
  console.log(`  specbase new change <change-id> --store ${payload.store.id}`);
  if (payload.git.is_repository) {
    const shareRemote = remotes?.canonical ?? remotes?.observed;
    console.log(
      shareRemote
        ? `Share it: teammates clone ${shareRemote} and run specbase store register <path>.`
...
function printListHuman(payload: StoreListOutput): void {
  if (payload.stores.length === 0) {
    ...
    console.log('');
    console.log('Next:');
    console.log('  specbase store setup team-context --path ~/specbase/team-context');
    console.log('  specbase store register /path/to/store');
    return;
  }

  console.log(`Specbase stores (${payload.stores.length})`);
  console.log('');
...
function formatSpecbaseRootHuman(store: StoreDoctorOutput['stores'][number]): string {
  if (store.specbase_root.healthy) return 'ok';
  if (store.specbase_root.present === false) return 'missing';
  if (store.specbase_root.present === null) return 'unknown';
  return 'incomplete';
}
...
function printDoctorHuman(payload: StoreDoctorOutput): void {
  ...
  for (const store of payload.stores) {
    ...
    console.log(`  Specbase root: ${formatSpecbaseRootHuman(store)}`);
...
export function registerStoreCommand(program: Command): void {
  ...
  store
    ...
    .option('--path <path>', 'Folder where the store should live (for example ~/specbase/<id>)')
  ...
  store
    ...
    .option('--yes', 'Confirm creating store identity metadata for a healthy Specbase root')
    .option('--json', 'Output as JSON')
  ...
  store.action(() => {
    ...
    if (operands.includes('--json')) {
      const message =
        attempted.length > 0
          ? `Unknown command '${attempted[0]}' for 'specbase store'. Store subcommands: ${storeSubcommandsLine}.`
          : `Missing subcommand for 'specbase store'. Store subcommands: ${storeSubcommandsLine}.`;
      printJson({
        status: [
          {
            severity: 'error',
    ...
    let example = 'specbase new change <change-id> --store <id>';
    if (!hasFlagLikeToken && attempted.length > 0 && lifecycleRedirects.has(attempted[0])) {
      if (attempted[0] === 'new') {
        ...
        example = `specbase new change ${changeId} --store <id>`;
      } else {
        example = `specbase ${attempted.join(' ')} --store <id>`;
      }
    }
    console.error(
      attempted.length > 0
        ? `Error: unknown command '${attempted[0]}' for 'specbase store'.`
        : "Error: missing subcommand for 'specbase store'."
    );
    console.error(

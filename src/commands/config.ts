...
import { resolvePlanningDirName } from '../core/config.js';
...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for Specbase',
  },
};
...
function maybeWarnProjectConfigDrift(
  ...
  const specbaseDir = path.join(projectDir, resolvePlanningDirName(projectDir));
  ...
  console.log(colorize('Warning: Global config is not applied to this project. Run `specbase update` to sync.'));
}

function printConfigProfileApplyGuidance(): void {
  console.log('Config updated. Run `specbase update` in your projects to apply.');
}
...
export function registerConfigCommand(program: Command): void {
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "specbase config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: specbase config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `specbase config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `specbase config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        // Check if inside a Specbase project
        ...
        if (fs.existsSync(specbaseDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx specbase update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `specbase update` in your other projects to apply.');
            } catch {
              console.error('`specbase update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }

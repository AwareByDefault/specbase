...
function worksetCliOpenerDisabledError(
  ...
  return new StoreError(
    ...
    {
      target: 'workset.tool',
      fix: `Open in VS Code or Cursor: specbase workset open ${name} --tool code`,
    }
  );
}
...
class WorksetCommand {
  async create(
    ...
    try {
      ...
      console.log(
        `Open it any time with: specbase workset open ${workset.name}`
      );
    } catch (error) {
  ...
  private async composeFromFlags(
    ...
    if (!name) {
      throw new StoreError('Pass a workset name.', 'workset_name_required', {
        target: 'workset.name',
        fix: 'specbase workset create <name> --member <path>',
      });
    }
    ...
    if (memberFlags.length === 0) {
      throw new StoreError(
        ...
        'workset_members_required',
        {
          ...
          fix: `specbase workset create ${name} --member <path> --member <name>=<path>`,
        }
      );
    }
  ...
  async list(options: { json?: boolean } = {}): Promise<void> {
    try {
      ...
      if (worksets.length === 0) {
        console.log(
          'No worksets saved. Create one with: specbase workset create'
        );
        return;
      }
    ...
    try {
      if (options.json) {
        throw new StoreError(
          ...
          {
            target: 'workset.tool',
            fix: 'Inspect worksets with: specbase workset list --json',
          }
        );
      }
      ...
      prepared = await withWorksetsLock(async (state): Promise<PreparedOpen> => {
        ...
        if (surviving.length === 0) {
          throw new StoreError(
            ...
            {
              target: 'workset.member',
              fix: `Recompose it: specbase workset remove ${name} --yes && specbase workset create ${name} --member <path>`,
            }
          );
        }
      ...
      if (toolId !== undefined) {
        ...
        if (!isInteractive()) {
          throw new StoreError(
            ...
            {
              target: 'workset.tool',
              fix: `specbase workset open ${name} --tool <id>`,
            }
          );
        }
        ...
        if (
          ...
          if (alternative !== null) {
            throw new StoreError(error.message, 'workset_launch_failed', {
              target: 'workset.tool',
              fix: `Run: specbase workset open ${name} --tool ${alternative}`,
            });
          }
        }
        throw error;
      ...
      if (!options.yes) {
        ...
        if (options.json || !isInteractive()) {
          throw new StoreError(
            ...
            {
              target: 'workset.name',
              fix: `specbase workset remove ${name} --yes`,
            }
          );
        }
...
export function registerWorksetCommand(program: Command): void {
  ...
  // Parsed at the group level so `specbase workset --json` keeps the
  ...
  workset.action(() => {
    ...
    const message =
      attempted.length > 0
        ? `Unknown command '${attempted[0]}' for 'specbase workset'. Workset subcommands: ${subcommandsLine}.`
        : `Missing subcommand for 'specbase workset'. Workset subcommands: ${subcommandsLine}.`;

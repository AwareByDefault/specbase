...
**Deferred to a follow-on (`sync-catalog-planes-on-update`):** an opt-in `openspec update` sync that offers to add schema-catalog planes missing from a config. It is split out because safely mutating an existing `config.yaml` must preserve the user's `context`/`rules`/comments — `serializeConfig` today rebuilds the file from scratch and would drop them — so it needs a YAML-preserving editor, a separate surface from this change. The seeding stands alone without it.

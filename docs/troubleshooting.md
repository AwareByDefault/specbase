...
Specbase runs on Node 20.19.0+. Check your version and upgrade if needed:

```bash
node --version
```

If you use bun to install Specbase, note that Specbase still *runs* on Node, so you need Node 20.19.0+ available on your `PATH` regardless. See [Installation](installation.md).

### `specbase init` didn't configure my AI tool
...
4. **Confirm the files exist.** For Claude Code, check that `.claude/skills/` contains `specbase-*` folders. Other tools use their own directories, all listed in [Supported Tools](supported-tools.md).

5. **Check you initialized this project.** Skills are written per project. If you cloned a repo or switched folders, run `specbase init` (or `specbase update`) there.
...
### `specbase validate` reports warnings or errors

Validation checks your specs and changes for structural problems. Read the message: it names the file and the issue.

```bash
specbase validate <name>           # validate one item
specbase validate --all            # validate everything
specbase validate --all --strict   # stricter checks, good for CI
```

Common causes are a missing required section (like a spec with no scenarios) or a malformed delta header. Fix the file and re-run. The [CLI reference](cli.md#specbase-validate) documents the output format.

### The AI created incomplete or wrong artifacts

The AI didn't have enough context. A few levers help:

- Add project context in `specbase/config.yaml` so your stack and conventions are injected into every request. See [Customization](customization.md#project-configuration).
...
1. **Wrong filename.** It must be `specbase/config.yaml`, not `.yml`.
...
```bash
specbase schemas                    # list available schemas
specbase schema which <name>        # see where a schema resolves from
specbase schema init <name>         # create a custom one
```
...
You're in CI or a non-interactive shell, and Specbase found old files to clean up but can't prompt you. Approve automatically:
...
Restart your IDE. Skills are detected at startup. If they still don't appear, run `specbase update` and check the file locations in [Supported Tools](supported-tools.md).

### My old `project.md` wasn't migrated

That's intentional. Specbase never deletes `project.md` automatically because it may hold context you wrote. Move the useful parts into `config.yaml`'s `context:` section, then delete it yourself. The [Migration Guide](migration-guide.md#migrating-projectmd-to-configyaml) walks through this, including a prompt you can hand to your AI to do the distilling.

## Still stuck?

- **Discord:** [discord.gg/YctCnvvshC](https://discord.gg/YctCnvvshC)
- **GitHub Issues:** [github.com/AwareByDefault/specbase/issues](https://github.com/AwareByDefault/specbase/issues)
- **From your terminal:** `specbase feedback "what went wrong"` opens an issue for you.

When you report a problem, include your Specbase version (`specbase --version`), your Node version (`node --version`), your AI tool, and the exact command and output. It makes help much faster.

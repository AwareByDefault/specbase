...
This guide explains the core ideas behind Specbase and how they fit together. For practical usage, see [Getting Started](getting-started.md) and [Workflows](workflows.md).

## Philosophy

Specbase is built around four principles:
...
**Fluid not rigid.** Traditional spec systems lock you into phases: first you plan, then you implement, then you're done. Specbase is more flexible — you can create artifacts in any order that makes sense for your work.

**Iterative not waterfall.** Requirements change. Understanding deepens. What seemed like a good approach at the start might not hold up after you see the codebase. Specbase embraces this reality.

**Easy not complex.** Some spec frameworks require extensive setup, rigid formats, or heavyweight processes. Specbase stays out of your way. Initialize in seconds, start working immediately, customize only if you need to.

**Brownfield-first.** Most software work isn't building from scratch — it's modifying existing systems. Specbase's delta-based approach makes it easy to specify changes to existing behavior, not just describe new systems.

## The Big Picture

Specbase organizes your work into two main areas:
...
Specbase aims to avoid bureaucracy. Use the lightest level that still makes the change verifiable.
...
```
specbase/changes/add-dark-mode/
...
Delta specs are the key concept that makes Specbase work for brownfield development. They describe **what's changing** rather than restating the entire spec.
...
```yaml
# specbase/schemas/spec-driven/schema.yaml
name: spec-driven
artifacts:
  - id: proposal
...
specbase schema fork spec-driven research-first
```

**Example custom schema:**

```yaml
# specbase/schemas/research-first/schema.yaml
name: research-first
artifacts:
  - id: research
...
│                                                                              │
│   ┌────────────────┐                                                         │
│   │  1. START      │  /spcb:propose (core) or /spcb:new (expanded)           │
│   │     CHANGE     │                                                         │
│   └───────┬────────┘                                                         │
│           │                                                                  │
│           ▼                                                                  │
│   ┌────────────────┐                                                         │
│   │  2. CREATE     │  /spcb:ff or /spcb:continue (expanded workflow)         │
...
| **Source of truth** | The `specbase/specs/` directory, containing the current agreed-upon behavior |

...
1. **Edit the file directly.** Artifacts are plain Markdown in `specbase/changes/<name>/`. Open `proposal.md`, `design.md`, `tasks.md`, or a delta spec under `specs/` in your editor and change it. Nothing else is required.
...
If you're using the expanded commands, the natural flow is: edit the artifact, then run `/spcb:continue` to pick up from the new state, or `/spcb:apply` to keep implementing against the updated plan. If you're on the default `core` commands, edit the artifact and run `/spcb:apply`; it reads the current files, so it builds against whatever the artifacts now say.
...
Concretely, after some `/spcb:apply` work:

- Want to re-examine the plan? Open the artifacts and read them, or run `specbase show <change>` in your terminal for a consolidated view.
- Found something to change? Edit the artifact (or ask the AI to), then continue.
- Want a structured check that the code matches the plan? Run `/spcb:verify` (expanded command). It reports completeness, correctness, and coherence without blocking anything. See [Workflows: Verify](workflows.md#verify-check-your-work).

There's no "review phase" to return to, because review is something you can do at any point, including after implementation.

## "I edited the code by hand. How do I reconcile that with Specbase?"
...
A fast way to surface mismatches is `/spcb:verify`: it reads your artifacts and your code and tells you where they diverge. Treat its output as a to-do list for reconciliation, then archive once they agree.
...
- **Explore first, then re-propose.** If the problem is that the idea itself is unclear, step back to `/spcb:explore`, think it through, and let a sharper proposal come out of that. See [Explore First](explore.md).
...
There's a full flowchart and worked examples in [Workflows: When to Update vs Start Fresh](workflows.md#when-to-update-vs-start-fresh) and a deeper treatment in [SPCB: When to Update vs. Start Fresh](spcb.md#when-to-update-vs-start-fresh).

## A note on tasks

`tasks.md` is a living checklist, not a frozen plan. As you implement, you can add tasks you discover, remove ones that turned out unnecessary, or reorder them. The AI checks items off as it completes them during `/spcb:apply`, and it resumes from the first unchecked task if you come back later. Editing the list mid-flight is expected.
...
- [Commands](commands.md) - `/spcb:continue`, `/spcb:apply`, and `/spcb:verify` in detail

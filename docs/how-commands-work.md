...
If you ever type `/spcb:propose` into your terminal and nothing happens, this page is why. You are talking to the wrong half of OpenSpec. Slash commands are not terminal commands. They are instructions you give to your AI coding assistant, in the same chat box where you'd normally type "add a login form."
...
```text
   ...
   ┌──────────────────────┐               ┌──────────────────────────────┐
   │  $ openspec init     │   installs    │  /spcb:propose add-dark-mode  │
   │  $ openspec list     │  ──────────►  │  /spcb:apply                  │
   │  $ openspec view     │   commands    │  /spcb:archive                │
   └──────────────────────┘    & skills   └──────────────────────────────┘
        run openspec here                       run /spcb:* here
```
...
3. Watch the autocomplete: if OpenSpec is installed, you'll see `/spcb:propose`, `/spcb:apply`, and friends appear as you type the slash.
...
There's also an **expanded** set for people who want finer control (`/spcb:new`, `/spcb:continue`, `/spcb:ff`, `/spcb:verify`, `/spcb:bulk-archive`, `/spcb:onboard`). You turn it on with `openspec config profile`, then apply it with `openspec update`.

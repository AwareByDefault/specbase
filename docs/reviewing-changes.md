...
Specbase's whole promise is that you and your AI **agree on what to build before any code is written.** That agreement only means something if you actually read what the AI drafted. This page is about the two minutes where you do that — what to open, in what order, and what to look for.
...
```
/spcb:propose ──► REVIEW THE PLAN ──► /spcb:apply ──► REVIEW THE CODE ──► /spcb:archive
                  (before any code)                    (/spcb:verify)
```

1. **After `/spcb:propose`** (or `/spcb:ff`), before `/spcb:apply` — read the plan while it's still just words.
2. **After building**, with `/spcb:verify` — check that the code actually did what the plan said.

The first review is the one that saves you the most, and the one people skip. This page spends most of its time there.

## Read it in this order

A change is a folder of plain Markdown in `specbase/changes/<name>/`. Read the files in the order that lets you quit earliest if something's wrong:
...
Once the work is built, `/spcb:verify` is your second review. It re-reads the artifacts and the code and reports mismatches across three dimensions:
...
`/spcb:verify` is in the expanded profile. If you don't have it, turn it on with `specbase config profile` (then `specbase update`), or just re-read the change and the diff yourself.
...
If all seven pass, run `/spcb:apply` with confidence. If any fail, that's not a setback — it's the two minutes doing its job.
